...
# ** Selenium을 사용하는 이유?
#  - Requests는 현재 URL의 정적 페이지 소스코드만 수집가능
...
# ** Selenium 사용 방법 2가지
#  1. 직접 다운로드
#     -URL: https://sites.google.com/chromiu.org/driver/
#  2. 실시간(코드) 다운로드
...
def review_collector(movie_code, last_date):
   ...
   # ex) 전체 리뷰: 187개
   # 올림[ (187 - 10) / 30(평점 더보기 한번에 뜨는 리뷰 수) = 5.9개 ] => 6
   # 수식 : 올림((187-10) / 30)
   ...
   for item in review_list:
      # 수집 리유 Data와 last_date() 비교
      # 다음 영화 리뷰 날짜 표시방법
      # 1. "조금전" => 1, 2, 3 번을 모두 4번 처럼 해결하기
      # 2. "?분전"
      # 3. "?시간전"
      # 4. "2023.11.29. 16:14
      ...
      print(f"    - 리뷰: {review_writer}")
      print(f"    - 날짜: {review_date}")

      #24시간 이내에 작성 된 리뷰의 날짜 -> 24시간전, 3시간전 -> 다음영화날짜( ex: 2023.11.17. 2:12 )
      ...
      if len(review_date) < 7:
         ...
         #4) 계산된 등록일자 날짜 포맷 병견(다음 영화 리뷰 날짜 포맷)
         review_date = review_date.strftime("%Y. %m. %d. %H:%M")

      print(f"    - 날짜: {review_date}")

      # MariaDB 저장( 제목, 리뷰, 평점, 작성자, 작서일자)
      data = {
         "title" : movie_title,
